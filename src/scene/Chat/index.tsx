...
import { AuthInfo, ChatInfo, GetChatMessagesResponse } from '../../mock/response';
...
interface ChatProps {
  chatMessagesResp: GetChatMessagesResponse;
  authInfo: AuthInfo,
  currentChatInfo: ChatInfo
}

const fillBizInstance2Store = (props: ChatProps, store: Store) => {
  // 这里做了一些数据上不兼容的转换。
  // 由于 pb 结构大量 optional 但是实际是有值的，所以将它深度遍历转换成required后再计算以免 ts 报错
  // 虽然 ts 这样检测失效了，但是 mst 在运行时会帮我们再检测一次是否传值不匹配
  const propsAsRequiredAllAttrs = props as SetRequiredExcludeKeys<ChatProps, never>;
  const { authInfo, currentChatInfo, chatMessagesResp: { entity: { chats, chatters, messages } } } = propsAsRequiredAllAttrs;
...
function ChatWithStore(props: ChatProps) {
  ...
  // messageItemIdTypeMapModel 属于只在 chat 场景下才有效的业务数据，所以定义在这个场景下面，并直接挂载到 ChatViewModel 下
  const messageItemIdTypeMapModelSnapshot: SnapshotIn<typeof ChatViewModel>['messageItemIdTypeMapModel'] = {};
  (props.chatMessagesResp.messageItems || []).forEach(messageItem => {
    messageItemIdTypeMapModelSnapshot[messageItem.itemId] = messageItem
  })
  const chatViewModel = store.createOrUpdate(ChatViewModel, {
    currentChat: props.currentChatInfo.id,
    messageItemIdTypeMapModel: messageItemIdTypeMapModelSnapshot
  })

  return (
      <ChatView vmInstance={chatViewModel}/>
  )
}
